...
if __name__ == "__main__":
    # ------------------------------------------------------------------
    # 1. Load annotated positives from all four phases
    # ------------------------------------------------------------------
    pos_sources = [
        ("P1", P1_POS_DIR, "*.jpg"),
        ("P2", P2_POS_DIR, "*.jpeg"),
    ...
    raw_positives = []
    for tag, d, pattern in pos_sources:
        matches = glob.glob(os.path.join(d, pattern))
        raw_positives.extend(matches)
        print(f"  {tag}: {len(matches)} candidate images in {d}")
    ...
    print(f"\nPositives verified : {len(verified_pos)} with labels confirmed")
    if missing_labels:
        print(f"Skipped (no label) : {len(missing_labels)}  (first 3: {missing_labels[:3]})")

    # ------------------------------------------------------------------
    # 2. Load P1 safe backgrounds (confirmed empty tiles)
    # ------------------------------------------------------------------
    bg_paths = glob.glob(os.path.join(P1_BG_DIR, "*.jpg"))
    print(f"\nP1 safe backgrounds: {len(bg_paths)}  (from {P1_BG_DIR})")

    # ------------------------------------------------------------------
    # 3. Load P2 FP negatives (Excel-listed hard negatives)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 4. Combine sources and assign stratification labels
    #    0 = positive (annotated)
    #    1 = negative (BG or FP) — both confirmed empty
    # ------------------------------------------------------------------
    all_images = verified_pos + bg_paths + fp_paths
    all_strata = ([0] * len(verified_pos)
                  + [1] * len(bg_paths)
                  + [1] * len(fp_paths))
    ...
    print(f"  Pos:Neg     : {len(verified_pos)}:{len(bg_paths) + len(fp_paths)}")

    # ------------------------------------------------------------------
    # 5. Preprocess in parallel (augment positives, copy negatives)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 6. Clear stale YOLO label cache
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 7. Stratified K-Fold — folds run in parallel across GPUs
