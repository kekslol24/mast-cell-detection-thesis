...
A model-size comparison (P3-E) substituted yolo11l (43 M parameters) for yolo11n under identical conditions on the P1–P8 corpus and produced R_Normal = 0.695, the lowest value recorded across the entire project (mechanism in §Model Selection and Architecture).
...
| **Mean <br> ± std** |  |  |  |  |  |  | **0.748 ± 0.079** | **0.872 ± 0.064** | **0.864 ± 0.023** | **0.853 ± 0.087** | **0.885 ± 0.035** | **0.821 ± 0.165** |
...
The normalized confusion matrix at best.pt is shown in @fig-p6a-confusion. Evaluated on the validation split (15% of training data).

![Normalized confusion matrix for P6-A (best.pt, evaluated on the val split). Rows are ground-truth classes; columns are predicted classes.](figures/p6a_confusion_matrix_normalized.png){#fig-p6a-confusion}
...
The final deployment model (P6-A) was subsequently applied to the complete P11 tile set. The resulting confidence distribution is shown in @fig-conf_p11.
...
The metric range across the P5-B groups is wide but structured, and each extreme is explained by corpus composition rather than model instability. G1's mAP50 of 0.766 is depressed by the long-standing P1 Normal fragility (P1's old Pos_neg annotation regime and only 10 Normal test instances in the group), not by a Phase 5 regression. G2's recall of 0.773 reflects that P2 and P8 are the two most Atypisch-dominant patients in the corpus, so the G2 test set is unusually sensitive to any single missed Atypisch cell. G3, G4, and G5 each exceed 0.89 recall, converging with the P4-B LOPO mean of 0.895 on the earlier 15-patient corpus. That convergence is itself a result: training on the expanded 30-patient set maintained performance while adding 20 new patients to the evaluation scope, with no distributional drift relative to the P1 to P15 results. **absatz stimmt noch nicht ganz**
...
**RQ2** asked whether the pipeline achieves rare-event detection without unmanageable false positives. It does, with the caveat that operating threshold selection is left to the clinic. Mast cells are present in only a small fraction of tiles in a full slide scan (~20'000 tiles per patient), and the confirmed-false-positive hard-negative training loop suppresses the most common error modes across annotation generations. The conservative P5-B estimate achieves R_Normal = 0.821 at a false-positive rate that is manageable within the MastCellDetector review workflow: the representative G3 confusion matrix records main-diagonal entries of 0.96 (Atypisch) and 0.88 (Normal) with cross-class error at 0.01. The out-of-sample confidence histogram from patient P11 shows a bimodal distribution separating the false-positive bulk from the true-positive peak, giving the hospital a principled basis for threshold selection without inheriting a single hard-coded engineering value.
...
The pipeline's errors cluster into four understood modes, none of which is a general detection failure.
