...
I would like to express my sincere gratitude to Dr. Stefan Glüge for setting up the project for this thesis and for his valuable support and guidance throughout the writing and compilation process of this thesis. My appreciation also goes to Robin Kosche for showing me around the USZ and introducing me to the subject as well as Prof. Dr. Dr. Stefan Balabanov for allowing this project to take place.
...
A model-size comparison (P3-E) substituted yolo11l for yolo11n under identical conditions on the P1–P8 corpus and produced R_Normal = 0.695, the lowest value recorded across the entire project (mechanism in §Model Selection and Architecture).
...
### Model Testing {#sec-model-testing}
...
A per-class recall extraction issue affected single-class holdout folds through Phase 4; affected folds are excluded from per-class mean calculations throughout. The issue is fully described, with the fix applied in Phase 5, in @sec-measurement-notes.
...
The model shipped with the final version of the desktop application is the P6-A deployment model, trained on the complete P1-P53 corpus minus P11 using the production recipe established across Phases 3 and 4.

The final deployment model (P6-A) was trained on 1'832 images across 34 annotated patients (P1–P53 minus P11), achieving val mAP50-95 = 0.842, Precision = 0.906, Recall = 0.921 at best.pt (early stopping at epoch 657, best epoch 607). Weights: `hpc/tinker_final_holdoutP11/final/weights/best.pt`.
...
The model-size comparison (P3-E, yolo11l) produced mAP50-95 = 0.745 ± 0.095, precision = 0.778 ± 0.134, recall = 0.785 ± 0.103, and R_Normal = 0.695 ± 0.144, the lowest R_Normal across all runs and 14.5 percentage points below P3-B.

**Phase 4.** Adding patients P9–P15 (424 new images, 293 new Normal annotations) expanded the corpus to 1'564 images and 15 LOPO folds. P4-A (degrees=0, the production recipe with the rotation variable inadvertently unset) achieved mAP50-95 = 0.793 ± 0.106, precision = 0.851 ± 0.097, recall = 0.904, R_Atypisch = 0.913, and R_Normal = 0.890. The R_Normal gain from corpus expansion (+9.5 pp over P3-A) exceeded the largest single hyperparameter gain observed in Phase 3 (+4.5 pp from `degrees=5` in P3-B).
...
Across 35 folds, post-hoc evaluation reported mAP50-95 = 0.858 ± 0.120, precision = 0.925 ± 0.128, recall = 0.951 ± 0.084, R_Atypisch = 0.920 ± 0.129 (n=20 folds with Atypisch ground truth), and R_Normal = 0.941 ± 0.119 (n=29 folds with Normal ground truth). Of the 35 folds, 19 reported mAP50 at the metric ceiling (0.995), corresponding to patients from the P16–P53 acquisition batch who contributed only one to three annotated images each. On a one-image test set, a single correct detection produces recall = 1.000 regardless of model generalization capacity, inflating the aggregate mean.
...
The grouped design was motivated in part by the small-sample sensitivity that makes individual patient holdouts structurally unreliable: a single missed cell in a one- or two-annotation test set shifts recall by 50 to 100 percentage points, as seen across the P5-A ceiling folds (see @sec-fold-patterns). Eight patients contributed exactly one annotated cell each (P24, P28, P29, P31, P39, P40, P45, P50); splitting a single annotation across train and holdout is methodologically incoherent, so these patients were always assigned to the training set across all five folds. Their annotations contribute to training in every fold and do not appear in any test split.
...
Training convergence for the P6-A deployment model is shown in @fig-p6a-results. The model trained on 1'832 images (P1–P53 minus P11) and converged at epoch 607 (early stopping, patience=50).

![Training curves for the P6-A final deployment model. Panels show box loss, classification loss, and DFL loss for training and validation splits; precision, recall, mAP50, and mAP50-95 across epochs.](figures/p6a_results.png){#fig-p6a-results}
...
**The P1 annotation-regime outlier.** P1 was annotated under the original Pos_neg directory structure before the CVAT-assisted workflow, and its holdout fold was the weakest in Phase 3 and Phase 4 (mAP50 = 0.584 under the 14-patient training set). The expanded corpus partly compensated for this distribution mismatch (P1 mAP50 recovered to 0.813 under the 34-patient P5-A training set), but the annotation-regime difference remains a known source of error and is the main driver of G1's depressed Normal recall in P5-B.

**The per-class recall measurement bug.** A positional-indexing error in the per-class extraction utility misreported R_Normal for single-class holdout folds (notably P13) through Phase 4; it is a measurement artefact, not a model error, and is documented in full in @sec-measurement-notes. Affected folds are excluded from per-class means, and the underlying detections are correct.
