...
Performance metrics for Phase 3 and Phase 4 runs are reported from the evaluation calculation at the end of each training run which are computed from every single LOPO training / validation fold. Phase 1 and Phase 2 results derive from held-out validation folds under 5-fold stratified cross-validation and are not directly comparable to Phase 3–4 LOPO estimates, for the reasons described in the [Model Testing](#model-testing) section. All values quoted in this section refer to cross-fold means unless stated otherwise.

**Phase 1.** The P1-baseline achieved validation mAP50 = 0.800 ± 0.129, mAP50-95 = 0.649 ± 0.131, precision = 0.768 ± 0.117, and recall = 0.719 ± 0.178. Cross-fold variance was high: Fold 1 reached mAP50-95 = 0.805 while Fold 5 collapsed at epoch 29 (recall = 0.432). No per-class breakdown was available as P1 contained only Atypisch annotations.

**Phase 2.** The 3×4 factorial DoE grid (11 cells, FP oversampling ∈ {1×, 2×, 3×} crossed with background ratio ∈ {0, 1, 2, 3}) found no statistically distinguishable difference across compositions: cell-level mAP50-95 variation was smaller than the within-cell cross-fold standard deviation. Background tiles decreased performance at every FP oversampling level. The fp=1, bgr=0 configuration (P2-H) was the grid winner at every metric, achieving mAP50 = 0.604 ± 0.194, mAP50-95 = 0.494 ± 0.166, precision = 0.934 ± 0.057, and recall = 0.537 ± 0.210.

**Phase 3.** The structural pivot to 8-fold LOPO on 8 patients with on-the-fly per-epoch augmentation produced the largest single performance step in the project. P3-A achieved mAP50 = 0.844 ± 0.137, mAP50-95 = 0.718 ± 0.137, precision = 0.764 ± 0.138, and recall = 0.840 ± 0.122, a 30.3 percentage point improvement in recall over P2-H (0.537). The cross-fold standard deviation dropped from 0.166 to 0.137 despite the harder evaluation protocol.

The tinkering matrix (P3-B, 14 of 24 cells completed) identified `degrees=5` rotation augmentation as the dominant training-time lever. The winning configuration (`degrees=5, dfl=1.5, freeze=10, label_smoothing=0.0`) achieved mAP50 = 0.864, mAP50-95 = 0.741 ± 0.122, precision = 0.812 ± 0.153, recall = 0.866 ± 0.131, R_Atypisch = 0.892 ± 0.173, and R_Normal = 0.840 ± 0.176. `dfl=2.0` consistently regressed R_Normal relative to `dfl=1.5` across all degree and freeze combinations; `freeze=0` underperformed `freeze=10` in every recall-relevant slice; label smoothing was empirically inert (bit-identical fold metrics at `ls=0.0` and `ls=0.1`). These findings defined the production recipe (`degrees=5, dfl=1.5, freeze=10, lr0=0.001, cls=1.0`) adopted for all subsequent phases.
...
The model-size comparison (P3-E, yolo11l) produced mAP50 = 0.855 ± 0.103, mAP50-95 = 0.745 ± 0.095, precision = 0.778 ± 0.134, recall = 0.785 ± 0.103, R_Atypisch = 0.875 ± 0.170 and R_Normal = 0.695 ± 0.144, the lowest R_Normal across all runs and 14.5 percentage points below P3-B.

**Phase 4.** Adding patients P9–P15 (424 new images, 293 new Normal annotations) expanded the corpus to 1'564 images and 15 LOPO folds. P4-A (degrees=0, the production recipe with the rotation variable inadvertently unset) achieved mAP50 = 0.903 ± 0.100, mAP50-95 = 0.793 ± 0.106, precision = 0.851 ± 0.097, recall = 0.904 ± 0.120, R_Atypisch = 0.913 ± 0.110, and R_Normal = 0.890 ± 0.194. The R_Normal gain from corpus expansion (+9.5 pp over P3-A) exceeded the largest single hyperparameter gain observed in Phase 3 (+4.5 pp from `degrees=5` in P3-B).
...
Across 35 folds, post-hoc evaluation reported mAP50 = 0.948 ± 0.098, mAP50-95 = 0.858 ± 0.120, precision = 0.925 ± 0.128, recall = 0.951 ± 0.084, R_Atypisch = 0.920 ± 0.129 (n=20 folds with Atypisch ground truth), and R_Normal = 0.941 ± 0.119 (n=29 folds with Normal ground truth). Of the 35 folds, 19 reported mAP50 at the metric ceiling (0.995), corresponding to patients from the P16–P53 acquisition batch who contributed only one to three annotated images each. On a one-image test set, a single correct detection produces recall = 1.000 regardless of model generalization capacity, inflating the aggregate mean.
...
for i in range(len(metrics)):
    ax.text(x[i] - w/2, p5a_mean[i] - 0.02,
            f"{p5a_mean[i]:.3f}\n±{p5a_std[i]:.3f}",
            ha='center', va='top', fontsize=7, color='white', fontweight='bold')
    ax.text(x[i] + w/2, p5b_mean[i] - 0.02,
            f"{p5b_mean[i]:.3f}\n±{p5b_std[i]:.3f}",
            ha='center', va='top', fontsize=7, color='white', fontweight='bold')
ax.set_xticks(x)
...
@fig-p5p6-comparison shows mAP50-95, Precision, and Recall for P5-A (35-fold LOPO, test split), P5-B (5-group CV, test split), and P6-A (final deployment model, validation split). P6-A has no held-out test set as it was trained on all annotated patients minus P11.

```{python}
#| label: fig-p5p6-comparison
#| fig-cap: "Performance comparison across P5-A (35-fold LOPO, test), P5-B (5-group CV, test), and P6-A (final model, val). Error bars show std across folds, clipped at 1.0. P6-A is a single training run with no held-out test set."
#| fig-width: 1
#| fig-height: 1.5
...
for i, (label, color) in enumerate(zip(means.keys(), colors)):
    ...
    err_up = [min(si, 1.0 - vi) for vi, si in zip(m, s)]
    ax.bar(x + (i - 1) * w, m, w,
           label=label, color=color, alpha=0.85,
           yerr=[s, err_up], capsize=4, error_kw={"linewidth": 1})
    for j in range(len(metrics)):
        txt = f"{m[j]:.3f}\n±{s[j]:.3f}" if s[j] > 0 else f"{m[j]:.3f}"
        ax.text(x[j] + (i - 1) * w, m[j] - 0.02, txt,
                ha='center', va='top', fontsize=7, color='white', fontweight='bold')
